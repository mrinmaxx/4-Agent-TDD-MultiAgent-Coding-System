...
import graph_pipeline as gp  # 3-agent LangGraph pipeline (planner->implementer->tester loop)
...
_graph_queue: "queue.Queue | None" = None  # events for the currently-open /graph_stream
...
def _run_graph_job(task: str, on_event) -> None:
    """Run the 3-agent LangGraph pipeline, stream node events, then persist artifacts."""
    try:
        final = gp.run(task, on_event=on_event)
        dest = (RESULTS_DIR.parent / "three_agent_results" / f"{time.strftime('%Y%m%d-%H%M%S')}-{_slug(task)}").resolve()
        dest.mkdir(parents=True, exist_ok=True)
        for fn in ("PLAN.md", "solution.py", "test_self.py", "test_report.json"):
            src = gp.WORK / fn
            if src.exists():
                shutil.copy2(src, dest / fn)
        on_event({"type": "saved", "dir": str(dest),
                  "all_passed": bool(final.get("test_results", {}).get("all_passed")),
                  "retry_count": final.get("retry_count", 0)})
    except litellm.exceptions.RateLimitError:
        on_event({"type": "error", "kind": "rate_limit",
                  "message": "Groq free-tier rate limit (429) hit during the loop — this is the TPM cap, not a bug. Retry shortly."})
    except Exception as e:
        on_event({"type": "error", "message": f"{type(e).__name__}: {e}"})
    finally:
        on_event({"type": "end"})
...
def run_graph_route():
    ...
    q: queue.Queue = queue.Queue()
    _graph_queue = q
    threading.Thread(target=_run_graph_job, args=(task, q.put), daemon=True).start()
    return jsonify({"ok": True})
